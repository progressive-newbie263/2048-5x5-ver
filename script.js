...
let moveFactor = 4; 
let options = [ 
	2, 4, 8, 2, 4, 8, 2, 2, 4, 4, 2, 8, 2, 2, 4, 4, 2, 2, 4, 8, 2, 2, 8, 2, 4, 8
]; 
let matrix = []; 
let prevMatrix; 
...
let rowIdx2 = Math.floor(Math.random() * 5); 
let colIdx2 = Math.floor(Math.random() * 5); 

if (rowIdx === rowIdx2 && colIdx === colIdx2) { 
	rowIdx2 = Math.floor(Math.random() * 5); 
	colIdx2 = Math.floor(Math.random() * 5); 
} 
...
function gameOver(status) { 
	if (status === "Win") { 
		...
		result.style.color = "rgb(45, 104, 231)"; 
	} else { 
		result.innerText = "You Loose!!!"; 
		result.style.color = "rgb(231, 104, 45)"; 
	} 
} 
